...
from ..misc import PickleSerializer
...
class CacheClass(BaseCache):
    ...
    def get(self, key, default=None, version=None):
        ...
        if value is None:
            return default
        result = self.serializer.loads(force_bytes(value))
        return result
    ...
    def set(self, key, value, timeout=DEFAULT_TIMEOUT, version=None, client=None, _add_only=False):
        ...
        result = self._set(key, self.serializer.dumps(value), timeout, client, _add_only)
        return result
    ...
    def get_many(self, keys, version=None):
        ...
        results = self._mget(new_keys)
        for key, value in zip(new_keys, results):
            if value is None:
                continue
            value = self.serializer.loads(force_bytes(value))
