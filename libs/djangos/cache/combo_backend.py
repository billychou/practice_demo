...
from .sentinel_backend import SentinelRedisCache
from ..misc import get_cache


class ComboCache(SentinelRedisCache):
    """
    多级复合缓存:
    ...
    eg：
        ...
        # 复合缓存配置
        'combo-local': {
            'BACKEND': 'utils.custom_cache.ComboCache', #backend name
    ...
    def __init__(self, name, params):
        ...
        super(ComboCache, self).__init__(name, params)
    ...
    def get(self, key, default=None, version=None, acquire_lock=True, timeout_local=DEFAULT_TIMEOUT):
        ...
        try:
            # 尝试从remote cache 读取
            remote_value = super(ComboCache, self).get(key, version=version)
            ...
            if (remote_value is not None) or self.has_key(key, version=version):
    ...
    def set(self, key, value, timeout=DEFAULT_TIMEOUT, version=None, timeout_local=DEFAULT_TIMEOUT):
        ...
        # 2. set remote
        try:
            super(ComboCache, self).set(key, value, timeout, version)
        except Exception as ex:
            self._printexc(ex)
